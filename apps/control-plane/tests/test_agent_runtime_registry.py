...
(base_url/directory/runtime_id/binding_capability_epoch), attests the runtime
through the supervisor's fail-closed ``register``/``accept_activation`` gates,
refuses a second binding that would share a runtime endpoint (plan gate 6),
and owns the per-binding ``AgentPipelineService`` lifecycle (``start_all`` /
``stop_all`` / ``pipeline_for``).
"""
...
class FakeRuntimeClient:
    ...
    async def restart(
        ...
        return await self.health(RuntimeRef(RUNTIME_REF))
...
async def test_start_all_activates_and_starts_each_binding(
    ...
    # The settings-based default provider serves ONE binding: a multi-binding
    # test injects a provider with per-binding directories (M4.5 spec §6).
    def provider(runtime_ref: RuntimeRef) -> tuple[str, str]:
        return (f"http://endpoint/{runtime_ref}", f"/workspaces/{runtime_ref}")
...
# ---------------------------------------------------------------------------
# Tests: plan gate 6 - a shared runtime endpoint is refused fail closed.
# ---------------------------------------------------------------------------


async def test_second_binding_sharing_settings_endpoint_is_refused(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    # The default provider resolves every runtime to the same base_url +
    # directory, so a second binding must never be admitted: it would observe
    # the first binding's events on the shared global stream (plan gate 6).
    registry = make_registry(settings, repositories, hub)
    first = make_binding(runtime_ref="runtime-1")
    second = make_binding(runtime_ref="runtime-2")

    await registry.start_all([first, second])

    assert registry.pipeline_for(first.id) is not None
    assert registry.pipeline_for(second.id) is None
    assert second.id in registry.unavailable_bindings
    assert "same runtime endpoint" in registry.unavailable_bindings[second.id]
    await registry.stop_all()


async def test_injected_provider_sharing_one_endpoint_is_refused(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    # The guard is not specific to the settings default: any provider that
    # maps two different bindings onto one (base_url, directory) pair is
    # refused, because directory isolation is the two-binding boundary.
    def provider(runtime_ref: RuntimeRef) -> tuple[str, str]:
        return ("http://shared", "/workspace")

    registry = make_registry(settings, repositories, hub, endpoint_provider=provider)
    first = make_binding(runtime_ref="runtime-1")
    second = make_binding(runtime_ref="runtime-2", runtime_epoch=2)
...
async def test_two_bindings_on_distinct_endpoints_do_not_conflict(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    # Two containers that happen to share a directory name are isolated by
    # their base_url: only the full (base_url, directory) pair is guarded.
    def provider(runtime_ref: RuntimeRef) -> tuple[str, str]:
        return (f"http://endpoint-{runtime_ref}", "/workspace")
...
async def test_same_binding_rebuild_is_not_a_conflict(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    ...
    await registry.build_pipeline(binding)  # same binding: no endpoint conflict

    assert registry.pipeline_for(binding.id) is not None
    await registry.stop_all()  # closes the currently mapped (second) adapter
    # The superseded first adapter is no longer registry-owned; the test
    # closes it explicitly so no fake client leaks.
    await created[0].close()
    assert all(adapter.closed for adapter in created)


# ---------------------------------------------------------------------------
# Tests: supervisor register attestation (fail closed).
# ---------------------------------------------------------------------------


async def test_build_pipeline_drives_supervisor_register_attestation(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    supervisor = make_supervisor(ready=True, epoch=2)
    registry = make_registry(settings, repositories, hub, supervisor=supervisor)
    binding = make_binding(runtime_epoch=2)

    pipeline = await registry.build_pipeline(binding)

    assert registry.pipeline_for(binding.id) is pipeline
    # The supervisor now attests the runtime with the binding's identity.
    assert supervisor.accept_activation(RuntimeRef(RUNTIME_REF), 2) is True
    assert supervisor.accept_tool_call(RuntimeRef(RUNTIME_REF), 2) is True
    await registry.stop_all()
...
async def test_not_ready_runtime_is_rejected_fail_closed(
    ...
    supervisor = make_supervisor(ready=False)
    registry = make_registry(settings, repositories, hub, supervisor=supervisor)
    binding = make_binding()
...
async def test_epoch_mismatch_is_rejected_fail_closed(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    ...
    assert registry.pipeline_for(binding.id) is None
    assert binding.id not in registry.unavailable_bindings  # recorded by start_all
...
async def test_unknown_runtime_is_rejected_fail_closed(
    ...
    await client.cleanup(RuntimeRef(RUNTIME_REF))
...
async def test_start_all_records_supervisor_rejection_per_binding(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    supervisor = make_supervisor(ready=False)
    registry = make_registry(settings, repositories, hub, supervisor=supervisor)
    ...
    await registry.start_all([broken])

    assert registry.pipeline_for(broken.id) is None
    assert broken.id in registry.unavailable_bindings
    assert "attestation failed" in registry.unavailable_bindings[broken.id]
    await registry.stop_all()


async def test_post_attestation_activation_rejection_closes_adapter(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    class RejectActivationSupervisor(SupervisorConnector):
        """Attests on register but keeps the activation gate closed."""
    ...
    registry = make_registry(
        ...
        repositories,
        hub,
        supervisor=RejectActivationSupervisor(),
    ...
    # The attestation succeeded, the adapter was built, and the rejection
    # closed it (no leaked HTTP client) without mapping the binding.
...
async def test_binding_without_capability_ref_fails_closed(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    created: list[FakeAdapter] = []
    ...
    with pytest.raises(RuntimeNotReadyError):
        await registry.build_pipeline(make_binding(capability_ref=None))

    # The capability guard fires before any adapter is allocated.
    assert created == []
...
async def test_stop_all_stops_pipelines_and_closes_every_adapter(
    ...
    registry = make_registry(
        settings, repositories, hub, endpoint_provider=provider, adapter_factory=factory
    )
...
async def test_supervisor_register_receives_binding_identity(
    settings: Settings, repositories: RepositoryBundle, hub: AgentStreamHub
) -> None:
    supervisor = make_supervisor(epoch=1)
    registry = make_registry(settings, repositories, hub, supervisor=supervisor)
    binding = make_binding(runtime_epoch=1, capability_ref=CAPABILITY_REF)
