...
- Protocol revision: the MCP Streamable HTTP specification revision
  `2025-11-25` (plan §23 reference) is the pinned baseline; the actual
  negotiated revision is established by the `initialize` handshake and is
  verified against the pinned OpenCode image + SDK pair by the M4 contract
  tests (§10.1). B does not claim compatibility with every historical or draft
  revision.
...
- Session lifecycle: `initialize` → server response → `notifications/initialized`
  → `tools/list` → per-call traffic; the server-issued `Mcp-Session-Id` is
  echoed on subsequent requests; shutdown follows the drain/quiesce sequence
  (§6.2.1).
- Notification behavior: client-to-server `notifications/initialized` and
  `notifications/cancelled`; server-to-client `notifications/message`,
  `notifications/tools/list_changed`, and progress notifications are expected
  and handled; unknown notifications fail closed.
...
## 7. Recorded unknowns and discrepancies
...
```json termflow-pin-contract
{
  ...
  "mcp_handshake": {
    "protocol_revision": "2025-11-25 (MCP Streamable HTTP); negotiated via initialize; exact negotiated revision verified against the pinned OpenCode image + mcp SDK (>=2.0,<3) by the M4 contract tests; concrete value unknown until then",
    ...
    "session_lifecycle": "initialize, notifications/initialized, tools/list, Mcp-Session-Id echoed, drain/quiesce on shutdown",
    "notification_behavior": "client sends notifications/initialized and notifications/cancelled; server notifications/message and tools/list_changed expected; unknown notifications fail closed"
  },
