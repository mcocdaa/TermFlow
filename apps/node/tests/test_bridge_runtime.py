...
@pytest.mark.asyncio
async def test_unavailable_replay_sends_stream_gap_without_auto_capture() -> None:
    buffers = OutputBuffers(max_bytes_per_pane=1024)
    transport = FakeTransport(buffers)
    instance_id = uuid4()
    runtime = BridgeRuntime(
    ...
    gap = StreamGapPayload.model_validate(transport.messages[-1].payload)
    assert gap.pane_id == "%1"
