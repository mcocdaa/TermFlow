...
test('approves a pending device code from settings and refreshes authorized clients', async ({ page }, testInfo) => {
  ...
  await revokeDialog.getByLabel('管理员令牌').fill(adminToken)
...
test('uses env-authoritative relay settings and the complete authenticator lifecycle', async ({ page }, testInfo) => {
  ...
  await enableDialog.getByLabel('管理员令牌').fill(adminToken)
  await enableDialog.getByLabel('双重验证码').fill(totpForCounter(setupKey, enableCounter))
  ...
  await disableDialog.getByLabel('管理员令牌').fill(adminToken)
  await disableDialog.getByLabel('双重验证码').fill(totpForCounter(setupKey, loginCounter + 1))
