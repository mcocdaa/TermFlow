...
test('uses the real dashboard, themes, terminal transport, and responsive controls', async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name !== 'desktop') {
    ...
    expect(Math.abs(mobileLayout.keybarShellLeft - mobileLayout.viewportLeft)).toBeLessThanOrEqual(1)
    expect(Math.abs(mobileLayout.keybarShellRight - (mobileLayout.viewportLeft + mobileLayout.viewportWidth))).toBeLessThanOrEqual(1)
