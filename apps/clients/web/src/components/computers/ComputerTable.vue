<template>
  <div class="computer-table" role="table" aria-label="已注册 Computers">
    ...
    <article v-for="computer in computers" :key="computer.computer_id" class="computer-table-row" role="row">
      <div role="cell"><ComputerNameEditor :computer-id="computer.computer_id" :display-name="computer.display_name" @updated="computer.display_name = $event" /><span class="muted">{{ computer.hostname }}</span></div>
      <div role="cell"><strong>{{ computer.platform }}</strong><span class="muted">TermFlow {{ computer.client_version }}</span></div>
      <div role="cell"><strong>{{ computer.online_term_count }} 个在线 Term</strong><StatusPill :online="computer.online" /></div>
      <div role="cell"><span>注册：<time :datetime="computer.registered_at">{{ format(computer.registered_at) }}</time></span><span v-if="computer.last_seen_at">最近：<time :datetime="computer.last_seen_at">{{ format(computer.last_seen_at) }}</time></span></div>
    </article>
  </div>
</template>
