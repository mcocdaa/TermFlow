<template>
  <div v-if="panes.length" class="titlebar-menu pane-focus-menu">
    <button ref="trigger" data-action="toggle-pane-focus-menu" class="titlebar-button" :class="{ 'is-open': open }" type="button" :aria-expanded="open" aria-haspopup="menu" @click="$emit('update:open', !open)"><Focus :size="16" aria-hidden="true" /><span>聚焦 Pane</span><ChevronDown class="menu-chevron" :size="15" aria-hidden="true" /></button>
