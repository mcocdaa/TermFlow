...
describe('TerminalView', () => {
  it('uses a terminal-only shell with back, editable Term name, Computer, and connection status', async () => {
    ...
    expect(wrapper.get('[data-term-name]').element.tagName).toBe('BUTTON')
    ...
    canvas.vm.$emit('status', 'connected')
    await flushPromises()

    await wrapper.get('[data-action="toggle-display-menu"]').trigger('click')
    expect(wrapper.get('[role="menu"][aria-label="终端显示比例"]')).toBeTruthy()
    await wrapper.get('[data-action="toggle-tmux-menu"]').trigger('click')
    expect(wrapper.find('[role="menu"][aria-label="终端显示比例"]').exists()).toBe(false)
    expect(wrapper.get('[role="menu"][aria-label="tmux 操作"]')).toBeTruthy()
    await wrapper.get('[data-action="toggle-pane-focus-menu"]').trigger('click')
    expect(wrapper.find('[role="menu"][aria-label="tmux 操作"]').exists()).toBe(false)
    expect(wrapper.get('[role="menu"][aria-label="聚焦 Pane"]')).toBeTruthy()
    expect(wrapper.get('[data-action="toggle-display-menu"]').attributes('aria-expanded')).toBe('false')
    expect(wrapper.get('[data-action="toggle-tmux-menu"]').attributes('aria-expanded')).toBe('false')
    expect(wrapper.get('[data-action="toggle-pane-focus-menu"]').attributes('aria-expanded')).toBe('true')

    await wrapper.get('[data-action="toggle-display-menu"]').trigger('click')
    expect(wrapper.find('[role="menu"][aria-label="聚焦 Pane"]').exists()).toBe(false)
