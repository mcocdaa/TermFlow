...
def test_full_verification_checks_source_build_compose_configuration() -> None:
    ...
    assert "OPENCODE_SERVER_USERNAME=" in verify
    assert "OPENCODE_SERVER_PASSWORD=" in verify
    assert "OPENCODE_MODEL_API_KEY=" in verify
...
def test_opencode_agent_service_is_isolated_and_hardened() -> None:
    ...
    # M4 exit: the image must be a real tag+digest pin — the unpullable
    # placeholder must never survive to a deployable compose file.
    assert "@sha256:" in agent["image"]
    assert "PLACEHOLDER" not in agent["image"]
    # The pinned image ships the fixed `guest` UID 405 (verified live in M4
    # exit; UID 10001 does not exist in the image); /home/opencode is a
    # tmpfs under the read-only rootfs and the data volume mounts inside it.
    ...
    assert any(mount.startswith("/home/opencode") for mount in agent["tmpfs"])
    # healthcheck must carry the OPENCODE_SERVER_* basic auth (the image
    # has no node/curl; busybox wget + grep are used instead).
    assert agent["healthcheck"]["test"][0] == "CMD-SHELL"
