...
def test_node_workflow_is_manual_and_reusable() -> None:
    ...
    dispatch_version = triggers["workflow_dispatch"]["inputs"]["version"]
    assert dispatch_version["description"].startswith("Build version override")
    assert dispatch_version["required"] is False
    assert dispatch_version["default"] == ""
    assert dispatch_version["type"] == "string"
