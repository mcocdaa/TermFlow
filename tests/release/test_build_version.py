...
def test_default_is_used_without_tag_or_environment() -> None:
    ...
    ) == BuildVersion(DEFAULT_BUILD_VERSION, f"v{DEFAULT_BUILD_VERSION}", False, True)
...
def test_prepare_cli_materializes_the_fixed_default(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stderr
    assert result.stdout == f"{DEFAULT_BUILD_VERSION}\n"
    assert verify_materialized_version(tmp_path, DEFAULT_BUILD_VERSION) == []
