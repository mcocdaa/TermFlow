<template>
  <div class="page settings-page totp-activation-view">
    <header class="page-heading">
      <div><p class="eyebrow">Two Factor Authentication</p><h1>双重因素认证</h1><p>使用你的验证器 App 完成绑定，再决定是否对新登录启用保护。</p></div>
    ...
    <section class="settings-panel totp-guide-card" aria-live="polite">
      ...
      <template v-else-if="setup">
        <div class="totp-setup-material">
          ...
          <div><h2>扫描二维码</h2><p>在验证器 App 中扫码，或手工输入下面的设置密钥。</p><code data-setup-key>{{ setup.setup_key }}</code></div>
        </div>
      ...
      <template v-else-if="!status.configured || reconfiguring">
        <div><h2>{{ status.configured ? '重新配置验证器' : '验证管理员身份' }}</h2><p>管理员 Token 只用于本次验证，不会保存在客户端。</p></div>
      ...
      <template v-else>
        <div><h2>验证器已绑定</h2><p>绑定完成。登录保护目前{{ status.enabled ? '已开启' : '未开启' }}。</p></div>
        <div class="security-setting-row">
          <div><strong>启用双重认证登录</strong><span>开启时还需输入一个尚未使用的新验证码。</span></div>
