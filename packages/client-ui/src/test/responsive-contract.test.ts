...
describe('responsive shell contract', () => {
  ...
  it('uses one portrait/landscape logic with coarse-pointer behavior and safe-area insets', () => {
    ...
    expect(appCss).toMatch(/\.bottom-toast\s*\{[^}]*position: fixed;[^}]*safe-area-inset-bottom/s)
    expect(appCss).toMatch(/@media \(max-width: 47\.99rem\)[\s\S]*\.bottom-toast\s*\{[^}]*inset-block-end: calc\(5rem \+ env\(safe-area-inset-bottom\)\);/)
  })
})
