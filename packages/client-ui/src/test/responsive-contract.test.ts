...
describe('responsive shell contract', () => {
  ...
  it('uses one portrait/landscape logic with coarse-pointer behavior and safe-area insets', () => {
    ...
    expect(appCss).toContain('.totp-guide-card { min-height: 22rem; align-content: center; }')
    expect(appCss).toContain('.security-setting-row > .security-setting-label { position: relative; display: inline-flex; align-items: center; gap: var(--space-1); white-space: nowrap; }')
