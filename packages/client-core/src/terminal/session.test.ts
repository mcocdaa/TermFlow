...
describe('TerminalSession', () => {
  it('closes a connection that resolves after its close event', async () => {
    const connection = new FakeConnection()
    let resolveConnection!: (value: TerminalConnection) => void
    let emit!: (event: TerminalTransportEvent) => void
    const transport: TerminalTransport = {
      connect: vi.fn((_request: TerminalConnectRequest, callback: (event: TerminalTransportEvent) => void) => {
        emit = callback
        return new Promise<TerminalConnection>((resolve) => { resolveConnection = resolve })
      }),
    }
    const scheduler = new FakeScheduler()
    const session = new TerminalSession('term', callbackSpies(), { transport, scheduler, createId: () => ACTION_ID })
    const pending = session.connect()
    emit({ type: 'close', code: 1006 })
  ...
  it('resets before accepting a replacement stream', async () => {
    const { transport, callbacks } = await setup()
    ready(transport)
  ...
  it('uses semantic action envelopes and forwards binding snapshots', async () => {
    const { session, transport, callbacks } = await setup()
    ready(transport)
    ...
    await session.sendAction('copy_mode', { targetPaneId: '%1' })
  ...
  it('drops input before ready and immediately after terminal.closed', async () => {
    const { session, transport } = await setup()
    await session.sendInput('before-ready')
    await session.sendAction('new_window')
    expect(transport.connections[0]?.binary).toHaveLength(0)
    ready(transport)
    await session.sendInput('ready')
    ...
    await session.sendInput('after-close')
    expect(transport.connections[0]?.binary).toHaveLength(1)
  })
  ...
  it('resumes the exact terminal, stream, and output sequence', async () => {
    const { transport, scheduler } = await setup()
    ready(transport)
  ...
  it('requests an explicit detach when disposed', async () => {
    const { session, transport } = await setup()
    ready(transport)
    await session.dispose()
  ...
  it('stops reconnecting and distinguishes authentication from Origin rejection', async () => {
  ...
  it('ignores malformed and unknown controls without logging them', async () => {
  ...
  it('ignores controls for a stale or not-yet-ready terminal identity', async () => {
    const { transport, callbacks } = await setup()
