...
if [[ "$#" -ne 1 || -z "${1}" ]]; then
  echo "usage: build-node-image.sh IMAGE" >&2
  exit 2
fi
IMAGE="${1}"
...
for ((attempt = 1; attempt <= MAXIMUM_ATTEMPTS; attempt += 1)); do
  if docker build \
    -f "${REPOSITORY_ROOT}/deploy/Dockerfile.node" \
    -t "${IMAGE}" \
  ...
  if ((status >= 125)); then
    echo "Node image build exited with terminal status ${status} and will not be retried." >&2
    exit "${status}"
  fi

  if ((attempt == MAXIMUM_ATTEMPTS)); then
    echo "Node image build failed after ${MAXIMUM_ATTEMPTS} attempts." >&2
    exit "${status}"
  fi

  echo "Node image build attempt ${attempt}/${MAXIMUM_ATTEMPTS} failed; retrying in ${RETRY_DELAY_SECONDS}s." >&2
